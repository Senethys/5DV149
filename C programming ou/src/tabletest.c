 ...
 */
void getInsertSpeed(table *t, int *keys, int *values, int n)
{
	unsigned long start;
	unsigned long end;
 ...
 */
void getRandomExistingLookupSpeed(table *t, int *keys, int n)
{
	unsigned long start;
	unsigned long end;
 ...
 *    n - the number of lookups to perform
 */
void getRandomNonExistingLookupSpeed(table *t, int *keys, int n)
{
	unsigned long start;
	unsigned long end;
 ...
 *    n - the number of lookups to perform
 */
void getSkewedLookupSpeed(table *t, int *keys, int n)
{
	unsigned long start;
	unsigned long end;
 ...
 *    values - a list of values to use
 */
void getRemoveSpeed(table *t, int *keys, int n)
{
	unsigned long start;
	unsigned long end;
 ...
 */
void testLookupExistingKey(table *t, char *key, char *value)
{
	char *returnValue = (char *)table_lookup(t, key);
	if (returnValue==NULL){
 ...
 * key is not found.
 */
void testLookupMissingKey(table *t, char *key)
{
	char *returnValue = (char *)table_lookup(t, key);
	if (returnValue!=NULL){
...
void testLookupSingleElement()
{
	...
	testLookupExistingKey(t, key1, value1);
...
void testInsertLookupDifferentKeys()
{
	...
	testLookupExistingKey(t, "key1", "value1");
	testLookupExistingKey(t, "key2", "value2");
	table_insert(t, key3, value3);
	testLookupExistingKey(t, "key1", "value1");
	testLookupExistingKey(t, "key2", "value2");
	testLookupExistingKey(t, "key3", "value3");
...
void testInsertLookupSameKeys()
{
	...
	table_insert(t, key1, value1);
	testLookupExistingKey(t, key, value1);
	table_insert(t, key2, value2);
	testLookupExistingKey(t, key, value2);
	table_insert(t, key3, value3);
	testLookupExistingKey(t, key, value3);
...
void testRemoveElementsSameKeys()
{
	...
	testLookupExistingKey(t, key2, "value23");
...
void speedTest(int n)
{
	...
	getInsertSpeed(t, keys, values, n);
	getRandomNonExistingLookupSpeed(t, keys, n);
	getRandomExistingLookupSpeed(t, keys, n);
	getSkewedLookupSpeed(t, keys, n);
	getRemoveSpeed(t, keys, n);
